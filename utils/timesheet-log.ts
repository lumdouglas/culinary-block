...
const LOG_FILE = path.join(LOG_DIR, 'timesheet-audit.ndjson');

export type TimesheetLogEntry =
  | { op: 'clock_in';              timesheetId: string; userId: string; clockIn: string }
  | { op: 'clock_out';             timesheetId: string; userId: string; clockOut: string }
