...
export default async function InvoicesPage() {
    ...
    if (!isAdmin) {
        invoicesQuery = invoicesQuery.eq('tenant_id', user?.id);
    }
            ...
            <div className="flex items-center justify-between mb-8">
                <div>
                    ...
                    <p className="text-slate-500">
                        {isAdmin ? "Manage billing and payments" : "Your invoices and payment history"}
                    </p>
                </div>
                {isAdmin && (
                        ...
                        {invoices?.length === 0 ? (
                            <TableRow>
                                <TableCell colSpan={isAdmin ? 6 : 5} className="text-center py-12 text-slate-500">
                                    {isAdmin ? "No invoices found. Create one to get started." : "No invoices yet."}
                                </TableCell>
                            </TableRow>
                        ) : (
