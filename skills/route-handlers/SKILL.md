...
name: route-handlers
description: >
  Next.js App Router Route Handler patterns for the culinary-block project.
  Use when creating or editing API endpoints (route.ts files), webhooks, or
  external-facing REST routes. Triggers on: "route handler", "api route",
  "route.ts", "webhook", "REST endpoint", "external API".
---
...
If the route handler accesses Supabase data, always authenticate first — even in routes you think are "internal". Call `supabase.auth.getUser()` before any `.from()` query.

See the **`supabase-postgres-best-practices`** skill for the required auth pattern and admin role-check pattern.