...
1. **Create `.env.example`** — document all required env vars so new developers can onboard
2. **Clean up root directory** — move `test-*.mjs` and debug scripts to `scripts/debug/` or delete them
3. **Deduplicate `approveApplication`** — consolidate `app/actions/applications.ts` into `app/actions/admin.ts`
4. **Uncomment admin role check** in `utils/supabase/middleware.ts` — currently any authenticated user can access `/admin/*`
5. **Wire up the AI chat assistant** — component and API route exist, just needs to be rendered on a page
6. **Add Stripe integration** — invoices are fully built; payment collection is the gap
7. **Set up Playwright** — `data-testid` attributes are already in place across components
8. **Build recurring booking UI** — the `rrule` column is in the schema waiting to be used
9. **Add a GitHub Actions workflow** — lint + build check on PRs
10. **Replace `README.md`** — swap the `create-next-app` boilerplate for real project documentation
