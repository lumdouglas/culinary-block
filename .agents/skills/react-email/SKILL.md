...
## This Project — Email Setup Already Done

React Email is already installed (`@react-email/components`, `@react-email/render`).
**Do NOT run `npx create-email` or create a separate `react-email-starter` project.**

Email templates live in **`components/emails/`**. Sending is done via **Nodemailer**, not Resend.
Set `NODEMAILER_*` env vars (see `.env.example`).

When adding a new email template:
1. Create it in `components/emails/YourTemplate.tsx`
2. Render it with `render(<YourTemplate .../>)` from `@react-email/render`
3. Pass the HTML string to the Nodemailer transport in `app/actions/`

---

## Installation (Generic — skip if using this project)
